...
controllerRYU=`cat IM.json | grep ietf-IM:Nos | cut -c 20-40` #RYU
controllerODL=`cat IM.json | grep ietf-IM:Nos | cut -c 43-63` #ODL
...
do
    ...
    ovs-vsctl --db=$db set-controller s$i $controllerODL
done
